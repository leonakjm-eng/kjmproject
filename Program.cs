...
namespace FishTankGame
{
    ...
    public class Fish
    {
        ...
        public void Move(Rectangle bounds)
        {
            ...
            {
                // Feeding Frenzy Mode: Move towards target at 3x speed
    ...
    public class GameForm : Form
    {
        ...
        private PointF? activeFoodTarget = null; // The position of the dropped food
        ...
        public GameForm()
        {
            ...
            this.Text = "Fish Tank Game - Feeding Frenzy";
        ...
        private void GameLoop(object sender, EventArgs e)
        {
            ...
            // 2. Check Feeding (Feeding Frenzy)
            if (activeFoodTarget.HasValue)
            {
                Fish winner = null;
                float foodRadius = 10f; // Assume food is size 20 (radius 10)

                foreach (var fish in fishes)
                {
                    float dx = fish.Center.X - activeFoodTarget.Value.X;
                    float dy = fish.Center.Y - activeFoodTarget.Value.Y;
                    float dist = (float)Math.Sqrt(dx * dx + dy * dy);

                    // Check if fish touches the food
                    if (dist < fish.Radius + foodRadius)
                    {
                        winner = fish;
                        break; // Only the first one eats
                    }
                }
                ...
                {
                    winner.Eat();
                    activeFoodTarget = null; // Food consumed

                    // Reset all fishes to random movement
                    foreach (var f in fishes)
                    {
                        f.TargetPosition = null;
                    }
                }
            }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // Draw Active Food Target (Feeding Frenzy)
            if (activeFoodTarget.HasValue)
            {
                float fx = activeFoodTarget.Value.X;
                float fy = activeFoodTarget.Value.Y;
                int fSize = 20;
                g.FillEllipse(Brushes.OrangeRed, fx - fSize/2, fy - fSize/2, fSize, fSize); // Distinct color
                g.DrawEllipse(Pens.DarkRed, fx - fSize/2, fy - fSize/2, fSize, fSize);
            }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            {
                // Simple hit test for the panel area
                // Assuming clicking anywhere in panel works if food is present
                isDragging = true;
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (isDragging)
            {
                ...
                {
                    // Feeding Frenzy Logic:
                    // 1. Decrement available food
                    foodCount--;

                    // 2. Set Active Target
                    activeFoodTarget = e.Location;

                    // 3. Signal all fishes to rush
                    foreach (var fish in fishes)
                    {
