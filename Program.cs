...
namespace FishTankGame
{
    ...
    public class Fish
    {
        ...
        public float Size { get; set; } // Changed to public set for Cloning
        public Color Color { get; set; } // Changed to public set for Cloning
        ...
        public Fish Clone()
        {
            Fish newFish = new Fish(this.X, this.Y);
            newFish.Size = this.Size;
            newFish.Color = this.Color;
            // Velocity remains random from constructor to ensure separation
            return newFish;
        }
    ...
    public class GameForm : Form
    {
        ...
        public GameForm()
        {
            ...
            this.Text = "Fish Tank Game - Evolution";
        ...
        private void CheckPredation()
        {
            ...
            {
                fishes.Remove(eaten);
                deathCount++; // Track casualties
            }

            // Check Defeat Condition (Death >= 5)
            if (deathCount >= 5)
            {
                EndGame("Game Over: Too many casualties...");
            }
        }
        ...
        private void EndGame(string message)
        {
            ...
            foodTimer.Stop();
            Invalidate();
            this.Update(); // Force redraw before showing message
            MessageBox.Show(message, "Game End", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // Draw HUD (Alive & Death)
            using (Font font = new Font("Arial", 16, FontStyle.Bold))
            {
                // Alive (Center)
                string aliveText = $"Alive: {fishes.Count}";
                SizeF aliveSize = g.MeasureString(aliveText, font);
                float aliveX = (WindowWidth - aliveSize.Width) / 2;
                g.DrawString(aliveText, font, Brushes.Black, aliveX, 20);

                // Death (Right)
                string deathText = $"Death: {deathCount}";
                SizeF deathSize = g.MeasureString(deathText, font);
                float deathX = WindowWidth - deathSize.Width - 10;
                g.DrawString(deathText, font, Brushes.Red, deathX, 20);
            }

            // Draw available food icons (Shifted down below text)
            int iconSize = 20;
            int gap = 10;
            int startX = 20;
            int foodY = 60; // Below the text
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (isDragging)
            {
                ...
                // Check Victory Condition (Alive >= 20)
                if (fishes.Count >= 20)
                {
                    EndGame("Victory! The Tank is Full!");
                }
