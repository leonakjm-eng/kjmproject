...
namespace FishTankGame
{
    ...
    public enum GameState { Playing, Collection, GameClear, GameOver }

    // Data for Serialization
    public class TrophyData
    {
        public float Size { get; set; }
        public List<int> Colors { get; set; } // Argb
    ...
    public class Fish
    {
        ...
        public List<Color> SkinLayers { get; set; } // For Trophy
        ...
        {
            if (X < bounds.Left) { X = bounds.Left; if (DX < 0) DX = -DX; }
            else if (X + Size > bounds.Right) { X = bounds.Right - Size; if (DX > 0) DX = -DX; }

            if (Y < bounds.Top) { Y = bounds.Top; if (DY < 0) DY = -DY; }
            else if (Y + Size > bounds.Bottom) { Y = bounds.Bottom - Size; if (DY > 0) DY = -DY; }
        }
        ...
        public void Draw(Graphics g)
        {
            ...
            {
                for (int i = 0; i < SkinLayers.Count; i++)
                {
                    ...
                    using (Brush b = new SolidBrush(SkinLayers[i]))
                    {
                        g.FillEllipse(b, X + offset, Y + offset, drawSize, drawSize);
                    }
                    g.DrawEllipse(Pens.Black, X + offset, Y + offset, drawSize, drawSize);
                }
            }
            else
            {
    ...
    public class GameForm : Form
    {
        ...
        private List<TrophyData> trophies = new List<TrophyData>();
        ...
        public GameForm()
        {
            this.Text = "Fish Tank Game - Final";
            ...
            // Buttons
            btnAuto = new Button { Text = "Auto", Location = new Point(325, 60), Size = new Size(50, 25), BackColor = Color.White, Font = new Font("Arial", 8) };
            btnAuto.Click += (s, e) => { isAutoPlay = !isAutoPlay; btnAuto.BackColor = isAutoPlay ? Color.LightGreen : Color.White; };
            this.Controls.Add(btnAuto);

            btnRetry = new Button { Text = "Retry", Location = new Point(380, 60), Size = new Size(50, 25), BackColor = Color.White, Font = new Font("Arial", 8) };
            btnRetry.Click += (s, e) => { RestartGame(); };
            this.Controls.Add(btnRetry);

            btnCollection = new Button { Text = "Col", Location = new Point(435, 60), Size = new Size(40, 25), BackColor = Color.White, Font = new Font("Arial", 8) };
            btnCollection.Click += ToggleCollection;
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    string txt = File.ReadAllText("highscore.txt");
                    int.TryParse(txt, out highScore);
                }

                if (File.Exists("trophies.json"))
                {
                    string json = File.ReadAllText("trophies.json");
                    trophies = JsonSerializer.Deserialize<List<TrophyData>>(json) ?? new List<TrophyData>();
                }
            }
            catch { /* Ignore errors */ }
        }
        ...
        private void SaveData()
        {
            try
            {
                ...
                string json = JsonSerializer.Serialize(trophies);
                File.WriteAllText("trophies.json", json);
            }
            catch { /* Ignore errors */ }
        }

        private void ToggleCollection(object sender, EventArgs e)
        {
            if (currentState == GameState.Collection)
            {
                currentState = GameState.Playing; // Or restore previous? For simplicity, Playing.
                // If we were GameClear, maybe Reset?
                // Let's assume toggle back resumes game if not over.
                // But simplified: Toggle resumes Play.
            }
            else
            {
        ...
        {
            // Cap target at 20
            return Math.Min(20, 6 + (currentLevel - 1));
        }
        ...
        private void StartLevel(int level)
        {
            ...
            // Speed Scaling: Clamp at 3.0
            float speedMult = Math.Min(3.0f, 1.0f + (level - 1) * 0.2f);
        ...
        private void TriggerGameClear()
        {
            ...
            // Create Trophy
            Random r = new Random();
            Fish trophy = new Fish(WindowWidth/2 - 100, WindowHeight/2 - 50, 0, Color.Gold);
            trophy.Size = 200;
            trophy.SkinLayers = new List<Color>();
            int layers = r.Next(3, 6);
            for(int i=0; i<layers; i++)
                trophy.SkinLayers.Add(Color.FromArgb(r.Next(256), r.Next(256), r.Next(256)));
            ...
            {
                Size = trophy.Size,
                Colors = trophy.SkinLayers.Select(c => c.ToArgb()).ToList(),
                Date = DateTime.Now.ToString("yyyy-MM-dd HH:mm")
            };
            trophies.Add(data);
            if (trophies.Count > 10) trophies.RemoveAt(0); // Max 10, remove oldest
            SaveData();

            Invalidate();
            MessageBox.Show("Congratulations! You cleared Level 20!", "Game Clear");
        }
        ...
        private void GameLoop(object sender, EventArgs e)
        {
            ...
            for (int i = fallingFoods.Count - 1; i >= 0; i--)
            {
                ...
                foreach(var fish in fishes)
                {
                    ...
                    if (dist < fish.Radius + food.Radius) { eater = fish; break; }
                }
        ...
        private void HandlePhysicsAndPredation()
        {
                ...
                for (int j = i + 1; j < fishes.Count; j++)
                {
                    ...
                    if (distSq < minDist * minDist)
                    {
                        ...
                        if (!eaten)
                        {
                            ...
                            float nx = dx / dist;
                            float ny = dy / dist;
                            float move = overlap * 0.1f;
                            f1.X -= nx * move; f1.Y -= ny * move;
                            f2.X += nx * move; f2.Y += ny * move;

                            float v1n = f1.DX * nx + f1.DY * ny;
                            float v2n = f2.DX * nx + f2.DY * ny;
                            float m1 = f1.Size; float m2 = f2.Size;
                            float v1n_new = (v1n * (m1 - m2) + 2 * m2 * v2n) / (m1 + m2);
                            float v2n_new = (v2n * (m2 - m1) + 2 * m1 * v1n) / (m1 + m2);
                            f1.DX += (v1n_new - v1n) * nx; f1.DY += (v1n_new - v1n) * ny;
                            f2.DX += (v2n_new - v2n) * nx; f2.DY += (v2n_new - v2n) * ny;
                        }
                    }
                }
            }
            ...
            if (deathCount >= 5)
            {
                ...
                SaveData(); // Save high score if improved (checked in StartLevel too but check here too?)
                // Actually StartLevel updates highscore when level starts.
                gameTimer.Stop();
                foodTimer.Stop();
                Invalidate();
                MessageBox.Show("Game Over: Too many casualties...", "Defeat");
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                g.Clear(Color.Black);
                g.DrawString("Trophy Collection", new Font("Arial", 20, FontStyle.Bold), Brushes.Gold, 150, 20);
                ...
                foreach(var t in trophies)
                {
                    ...
                    {
                        float drawSize = 40;
                        for(int i=0; i<t.Colors.Count; i++)
                        {
                            float s = drawSize * (1.0f - i*0.2f);
                            if (s <= 0) break;
                            float off = (drawSize - s)/2;
                            using(Brush b = new SolidBrush(Color.FromArgb(t.Colors[i])))
                                g.FillEllipse(b, x+off, y+off, s, s);
                            g.DrawEllipse(Pens.White, x+off, y+off, s, s);
                        }
                    }
                    g.DrawString(t.Date, SystemFonts.DefaultFont, Brushes.White, x, y + 45);
            ...
            using (Font font = new Font("Arial", 16, FontStyle.Bold))
            {
                ...
                // Best Score
                g.DrawString($"Best: {highScore}", new Font("Arial", 10), Brushes.Green, 300, 20); // Add Best

                // Death
                string deathText = $"Death: {deathCount}";
                SizeF deathSize = g.MeasureString(deathText, font);
                g.DrawString(deathText, font, Brushes.Red, WindowWidth - deathSize.Width - 10, 20); // Adjusted position
            }
            ...
            for (int i = 0; i < foodStorage.Count; i++)
            {
                ...
                Brush brush = (foodStorage[i] == FoodType.Slow) ? Brushes.Purple : Brushes.Orange;
            ...
            foreach (var food in fallingFoods)
            {
                Brush brush = (food.Type == FoodType.Slow) ? Brushes.Purple : Brushes.OrangeRed;
            ...
            if (isDragging)
            {
                Brush brush = (draggedFoodType == FoodType.Slow) ? Brushes.Purple : Brushes.Orange;
            ...
            if (currentState == GameState.GameClear)
            {
                ...
                g.DrawString(msg, f, Brushes.Gold, (WindowWidth - s.Width)/2, 150);
            }
        }
